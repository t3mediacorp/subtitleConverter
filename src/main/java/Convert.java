...
import com.wazeedigital.subtitle.FormatFactory;
	...
	public static void main(String[] args) {
		...
		if (args != null && args.length == 4) {
			...
			try {
				...
				ttff = FormatFactory.getFormatForFile(inputFormat);
				if (ttff == null) {
					throw new Exception(
				...
				if ("SRT".equalsIgnoreCase(outputFormat)) {
					...
					throw new Exception(
							"Unrecognized input format: " + outputFormat + " only [SRT,STL,SCC,XML,ASS] are possible");
				}
